...
class GameBoard:
    ...
    def __init__(self, title, game, size, bkColor='SteelBlue', buttonColor='LightSalmon'):
        ...
        self.up = Button(Point(buttonX + buttonWidth/2, buttonY), "^", partial(self.do, self.game.goUp), buttonColor)
        self.up.draw(self.window)
        self.down = Button(Point(buttonX + buttonWidth/2, buttonY + lineSpace*2), "V", partial(self.do, self.game.goDown), buttonColor)
        self.down.draw(self.window)
        self.left = Button(Point(buttonX + buttonWidth*3/8, buttonY + lineSpace), "<--", partial(self.do, self.game.goLeft), buttonColor)
        self.left.draw(self.window)
        self.right = Button(Point(buttonX + buttonWidth*5/8, buttonY + lineSpace), "-->", partial(self.do, self.game.goRight), buttonColor)
        self.right.draw(self.window)
        self.wayBack = Button(Point(buttonX + buttonWidth/2, buttonY + lineSpace*4), "Show the way back", partial(self.do, self.game.showWayBack), buttonColor)
        self.wayBack.draw(self.window)
        self.pickUp = Button(Point(buttonX + buttonWidth*1/4, buttonY + lineSpace*6), "Pick up", partial(self.do, self.game.pickUp), buttonColor)
        self.pickUp.draw(self.window)
        self.performTask = Button(Point(buttonX + buttonWidth*3/4, buttonY + lineSpace*6), "Perform task", partial(self.do, self.game.performTask), buttonColor)
        self.performTask.draw(self.window)
        self.quit = Button(Point(buttonX + buttonWidth*1/4, buttonY + lineSpace*8), "Quit", self.window.close, buttonColor)
        self.quit.draw(self.window)
        self.help = Button(Point(buttonX + buttonWidth*3/4, buttonY + lineSpace*8), "Help", self.nothing, buttonColor)
        ...
        #print("gameboard init done")

    def keypressed(self, ch):
        # This controls the gui rection to key presses
        if ch in ['w', 'W', u"\u2729"]:
            self.do(self.game.goUp)
